export const testimonials2 = [
  {
    id: 1,
    quote:
      "Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!",
    avatarSrc: "/images/avatar/user-7.jpg",
    productTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
    rating: 5,
    name: "Lisa K.",
    role: "Stylist",
  },
  {
    id: 2,
    quote:
      "Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!",
    avatarSrc: "/images/avatar/user-7.jpg",
    productTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
    rating: 5,
    name: "Lisa K.",
    role: "Stylist",
  },
];

export const testimonialData = [
  {
    stars: 5,
    review:
      "Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!",
    author: "Sybil Sharp",
    avatar: "/images/avatar/user-4.jpg",
    product: {
      title: "Contrasting sheepskin sweatshirt",
      price: "₹60.00",
    },
  },
  {
    stars: 5,
    review:
      "I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face.",
    author: "Mark G.",
    avatar: "/images/avatar/user-5.jpg",
    product: {
      title: "Contrasting sheepskin sweatshirt",
      price: "₹60.00",
    },
  },
  {
    stars: 5,
    review:
      "I love this shop! Top-quality products and incredibly friendly staff. They always go the extra mile to ensure I'm satisfied with my purchase.",
    author: "Emily S.",
    avatar: "/images/avatar/user-6.jpg",
    product: {
      title: "Contrasting sheepskin sweatshirt",
      price: "₹60.00",
    },
  },
  {
    stars: 5,
    review:
      "Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!",
    author: "Sybil Sharp",
    avatar: "/images/avatar/user-4.jpg",
    product: {
      title: "Contrasting sheepskin sweatshirt",
      price: "₹60.00",
    },
  },
];

export const testimonials3 = [
  {
    textTitle: "Radiant Glow Like Never Before",
    content:
      "I've tried countless skincare products, but nothing compares to the glow I got from this brand! My skin feels soft, hydrated, and absolutely radiant. The natural ingredients make all the difference. Highly recommend!",
    author: "Sybil Sharp",
    avatar: "/images/avatar/user-4.jpg",
    priceTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    textTitle: "Quality of Clothing!",
    content:
      "I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face.",
    author: "Mark G.",
    avatar: "/images/avatar/user-5.jpg",
    priceTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    textTitle: "Customer Service!",
    content:
      "I love this shop! The products are always top-quality, and the staff is incredibly friendly and helpful. They go out of their way to make sure that I'm satisfied with my purchase.",
    author: "Emily S.",
    avatar: "/images/avatar/user-6.jpg",
    priceTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    textTitle: "Quality of Clothing!",
    content:
      "I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face.",
    author: "Mark G.",
    avatar: "/images/avatar/user-5.jpg",
    priceTitle: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
];

export const testimonials4 = [
  {
    author: "Sybil Sharp",
    avatar: "/images/avatar/user-4.jpg",
    quote:
      '"Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!"',
    product: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    author: "Mark G.",
    avatar: "/images/avatar/user-5.jpg",
    quote:
      '"I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face."',
    product: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    author: "Emily S.",
    avatar: "/images/avatar/user-6.jpg",
    quote:
      '"I love this shop! Top-quality products and incredibly friendly staff. They always go the extra mile to ensure I\'m satisfied with my purchase."',
    product: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
  {
    author: "Sybil Sharp",
    avatar: "/images/avatar/user-4.jpg",
    quote:
      '"Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!"',
    product: "Contrasting sheepskin sweatshirt",
    price: "₹60.00",
  },
];

export const testimonials5 = [
  {
    id: 1,
    author: "Ananya Mehra",
    text: `"I have sensitive skin, and Beaubless products work like magic! My skin has never felt this soft and healthy. It’s become an essential part of my daily routine."`,
    avatar: "/images/avatar/user-default.jpg",
    product: "Hydrating Aloe Vera Gel",
    price: "₹349.00",
  },
  {
    id: 2,
    author: "Ritika Sharma",
    text: `"Beaubless is a blessing for my skin! I used to struggle with dryness and irritation, but their products feel so gentle and soothing. Love the glow it gives!"`,
    avatar: "/images/avatar/user-default.jpg",
    product: "Rose & Vitamin C Serum",
    price: "₹499.00",
  },
  {
    id: 3,
    author: "Priya Nair",
    text: `"I’ve tried many brands but nothing feels as natural and effective as Beaubless. My skin looks visibly clearer and feels refreshed every day!"`,
    avatar: "/images/avatar/user-default.jpg",
    product: "Gentle Foaming Face Wash",
    price: "₹299.00",
  },
];


export const testimonials6 = [
  {
    title: "Incredible Glow",
    text: `"My skin has never looked this radiant! The products feel so light and refreshing, and the results are amazing."`,
    author: "Sybil Sharp",
    delay: "0s",
    stars: 5,
  },
  {
    title: "Gentle & Effective",
    text: `"I have sensitive skin, and these products work perfectly! No irritation, just smooth, healthy skin. I finally found a brand I can trust!"`,
    author: "Mark G.",
    delay: "0.1s",
    stars: 5,
  },
  {
    title: "Amazing Quality",
    text: `"The foundation blends so well and feels so light. Luxury quality at such an affordable price!"`,
    author: "Emily S.",
    delay: "0.2s",
    stars: 5,
  },
  {
    title: "Pocket Friendly",
    text: `" Highly recommended. The quality of products is exceptional, and the prices are very reasonable!"`,
    author: "Sybil Sharp",
    delay: "0.3s",
    stars: 5,
  },
];

export const testimonials7 = [
  {
    delay: "0s",
    title: "Variety of Styles!",
    text: `"Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!"`,
    author: "Sybil Sharp",
  },
  {
    delay: "0.1s",
    title: "Quality of Clothing!",
    text: `"I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face."`,
    author: "Mark G.",
  },
  {
    delay: "0.2s",
    title: "Customer Service!",
    text: `"I love this shop! The products are always top-quality, and the staff is incredibly friendly and helpful. They go out of their way to make sure that I'm satisfied with my purchase."`,
    author: "Emily S.",
  },
  {
    delay: "0s",
    title: "Variety of Styles!",
    text: `"Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!"`,
    author: "Sybil Sharp",
  },
  {
    delay: "0.1s",
    title: "Quality of Clothing!",
    text: `"I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face."`,
    author: "Mark G.",
  },
  {
    delay: "0.2s",
    title: "Customer Service!",
    text: `"I love this shop! The products are always top-quality, and the staff is incredibly friendly and helpful. They go out of their way to make sure that I'm satisfied with my purchase."`,
    author: "Emily S.",
  },
  {
    delay: "0s",
    title: "Variety of Styles!",
    text: `"Fantastic shop! Great selection, fair prices, and friendly staff. Highly recommended. The quality of the products is exceptional, and the prices are very reasonable!"`,
    author: "Sybil Sharp",
  },
  {
    delay: "0.1s",
    title: "Quality of Clothing!",
    text: `"I absolutely love this shop! The products are high-quality and the customer service is excellent. I always leave with exactly what I need and a smile on my face."`,
    author: "Mark G.",
  },
  {
    delay: "0.2s",
    title: "Customer Service!",
    text: `"I love this shop! The products are always top-quality, and the staff is incredibly friendly and helpful. They go out of their way to make sure that I'm satisfied with my purchase."`,
    author: "Emily S.",
  },
];
